...
inline void AidenStyle()
{
	...
	colors[ImGuiCol_PopupBg]                = ImVec4(0.18f, 0.18f, 0.18f, 0.90f);
	...
	colors[ImGuiCol_TitleBgCollapsed]       = ImVec4(0.11f, 0.11f, 0.11f, 0.91f);
	...
	oConfig.m_oTabColorActive.Value = colors[ImGuiCol_ChildBg];
	oConfig.m_oTabColorNormal.Value = ImVec4(0.233f, 0.233f, 0.233f, 1.000f);
	oConfig.m_oTabColorBorder.Value = ImVec4(0.000f, 0.000f, 0.000f, 0.208f);
}
...
public:
	...
	virtual void OnGui()
	{
		...
		if (ImGui::SmallButton("Default##Color"))
		{
			...
			style.Colors[ImGuiCol_PopupBg] = ImVec4(0.1f, 0.1f, 0.1f, 0.9f);
			...
			//style.Colors[ImGuiCol_CloseButton] = ImVec4(0.50f, 0.50f, 0.90f, 0.50f); // Now uses regular button colors
		...
		if (ImGui::SmallButton("Dark##Color"))
		{
			...
			//style.Colors[ImGuiCol_CloseButton] = ImVec4(0.41f, 0.41f, 0.41f, 0.50f); // Now uses regular button colors
		...
		if (ImGui::SmallButton("Blue##Color"))
		{
			...
			//style.Colors[ImGuiCol_CloseButton] = ImVec4(0.93f, 0.94f, 0.95f, 0.16f); // Now uses regular button colors
		...
		if (ImGui::SmallButton("White##Color"))
		{
			oConfig.m_eTabColorMode = ImwWindowManager::E_TABCOLORMODE_BACKGROUND;
			ImGuiStyle& style = ImGui::GetStyle();
			style.Colors[ImGuiCol_Text] = ImVec4(0.03f, 0.02f, 0.02f, 1.00f);
			style.Colors[ImGuiCol_TextDisabled] = ImVec4(0.60f, 0.60f, 0.60f, 1.00f);
			...
			style.Colors[ImGuiCol_ResizeGripHovered] = ImVec4(1.00f, 1.00f, 1.00f, 0.60f);
			style.Colors[ImGuiCol_ResizeGripActive] = ImVec4(1.00f, 1.00f, 1.00f, 0.90f);
			//style.Colors[ImGuiCol_CloseButton] = ImVec4(0.41f, 0.75f, 0.98f, 0.50f); // Now uses regular button colors
		...
		ImGui::SameLine();
		if (ImGui::SmallButton("Aiden##Color"))
		{
			AidenStyle();
		}
...
public:
	...
	virtual void OnGui()
	{
		if (!IsClosable())
		{
			...
			ImGui::TextWrapped("To close me, click on button 'Close'");
			ImGui::Separator();
		}
...
{
public:
	MyImwWindow2(const char* pTitle = "Widget Gallery")
	{
		SetTitle(pTitle);
	}
	...
	virtual void OnGui()
	{
		static bool visibleDemoWindow = false;

		ImGui::Checkbox("Widget Gallery Window Visible", &visibleDemoWindow);

		if (visibleDemoWindow)
		{
...
static inline void MyShowExampleMenuFile()
{
...
class MyMainMenu : public ImwMainMenu
{
...
void InitSample()
{
	...
	oMgr.SetMainMenu(new MyMainMenu());
...
int main()
{
	...
	auto exepath = Handy::Platform::Paths::ThisExecutableFile();
	auto fontsdir = exepath.remove_filename().parent_path() / "Fonts";
	...
	io.Fonts->AddFontFromFileTTF((fontsdir / "Roboto-Light.ttf").u8string().c_str(), 16);
	io.Fonts->AddFontFromFileTTF((fontsdir / "Roboto-Regular.ttf").u8string().c_str(), 16);
	io.Fonts->AddFontFromFileTTF((fontsdir / "Roboto-Medium.ttf").u8string().c_str(), 16);
	...
	io.Fonts->AddFontFromFileTTF((fontsdir / "Input-Regular.ttf").u8string().c_str(), 12);
